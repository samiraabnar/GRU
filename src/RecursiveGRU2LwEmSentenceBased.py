...
class RecursiveGRU2LwEmSentenceBased(object):
    ...
    def __init__(self, input_dim,output_dim, hidden_dim= 128, bptt_truncate=-1):
        ...
        U_update[0] = np.random.uniform(-np.sqrt(1./self.embedding_dim), np.sqrt(1./self.embedding_dim), (self.hidden_dim, self.embedding_dim))
        W_update[0] = np.random.uniform(-np.sqrt(1./self.hidden_dim), np.sqrt(1./self.hidden_dim), (self.hidden_dim, self.hidden_dim))
        b_update[0] = np.zeros((self.hidden_dim))
        U_reset[0] = np.random.uniform(-np.sqrt(1./self.embedding_dim), np.sqrt(1./self.embedding_dim), (self.hidden_dim, self.embedding_dim))
        W_reset[0] = np.random.uniform(-np.sqrt(1./self.hidden_dim), np.sqrt(1./self.hidden_dim), (self.hidden_dim, self.hidden_dim))
        b_reset[0] = np.zeros((self.hidden_dim))
        U_candidate[0] = np.random.uniform(-np.sqrt(1./self.embedding_dim), np.sqrt(1./self.embedding_dim), (self.hidden_dim, self.embedding_dim))
        W_candidate[0] = np.random.uniform(-np.sqrt(1./self.hidden_dim), np.sqrt(1./self.hidden_dim), (self.hidden_dim, self.hidden_dim))
        b_candidate[0] = np.zeros((self.hidden_dim))



        V = np.random.uniform(-np.sqrt(1./self.hidden_dim), np.sqrt(1./self.hidden_dim), (self.output_dim, self.hidden_dim))
    ...
    def save_model_parameters_theano(model, outfile):
        np.savez(outfile,
            ...
            U_update=model.U_update[0].get_value(),
            U_reset=model.U_reset[0].get_value(),
            U_candidate=model.U_candidate[0].get_value(),
            W_update=model.W_update[0].get_value(),
            W_reset=model.W_reset[0].get_value(),
            W_candidate=model.W_candidate[0].get_value(),
            b_update=model.b_update[0].get_value(),
            b_reset=model.b_reset[0].get_value(),
            b_candidate=model.b_candidate[0].get_value(),
            V=model.V.get_value(),
    ...
    def load_model_parameters_theano(path):
        ...
        E, U_update, U_reset, U_candidate,W_update, W_reset,W_candidate,b_update,b_reset,b_candidate, V, ob = \
                           modelFile["Embedding"], modelFile["U_update"], modelFile["U_reset"], modelFile["U_candidate"], \
                                                   modelFile["W_update"],modelFile["W_reset"], modelFile["W_candidate"],\
                                                   modelFile["b_update"],modelFile["b_reset"],modelFile["b_candidate"],\
                                                   \
                                                   \
        ...
        sys.stdout.flush()
        model = GRU2LwEmSentenceBased(input_dim=E.shape[1],embedding_dim=E.shape[0], hidden_dim=U_update.shape[0],output_dim=V.shape[0])
        model.Embedding.set_value(E)
        model.U_update[0].set_value(U_update)
        model.U_reset[0].set_value(U_reset)
        model.U_candidate[0].set_value(U_candidate)
        model.W_update[0].set_value(W_update)
        model.W_reset[0].set_value(W_reset)
        model.W_candidate[0].set_value(W_candidate)
        model.b_update[0].set_value(b_update)
        model.b_reset[0].set_value(b_reset)
        model.b_candidate[0].set_value(b_candidate)
    ...
    def load_model_parameters_only(path):
        ...
        model["Embedding"] = E
        model["U_update"][0] = U_update
        model["U_reset"][0] = U_reset
        model["U_candidate"][0] = U_candidate
        model["W_update"][0] = W_update
        model["W_reset"][0] = W_reset
        model["W_candidate"][0] = W_candidate
        model["b_update"][0] = b_update
        model["b_reset"][0] = b_reset
        model["b_candidate"][0] = b_candidate
